...
- `GET /api/members` — returns every member with two derived fields:
  ...
  Rows are ordered by `last_activity_at DESC NULLS LAST, name`. The home page and TV view feature up to 5 members who have both non-null `last_activity_at` and `watching_count >= 6` (the threshold keeps featured cards from leading to thin profiles); the rest go behind a "Browse all members" disclosure.
